...
Extended with recommended operations (satellite skills) and enhanced checklists.
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # Search for recommended satellite operations
        ops_result = search(query, "operations", 3)
        ops = ops_result.get("results", [])
        recommended_operations = []
        for op in ops:
            recommended_operations.append({
                "name": op.get("Operation", ""),
                "description": op.get("Description", ""),
                "when_to_use": op.get("When To Use", "")
            })
...
def format_ascii_box(design_system: dict) -> str:
    ...
    recommended_ops = design_system.get("recommended_operations", [])
    ...
    if recommended_ops:
        lines.append("|  RECOMMENDED OPERATIONS:".ljust(BOX_WIDTH) + "|")
        for op in recommended_ops:
            op_name = op.get("name", "")
            op_desc = op.get("description", "")
            lines.append(f"|     - {op_name}".ljust(BOX_WIDTH) + "|")
            if op_desc:
                for line in wrap_text(op_desc, "|       ", BOX_WIDTH):
                    lines.append(line.ljust(BOX_WIDTH) + "|")
        lines.append("|" + " " * BOX_WIDTH + "|")
    ...
    checklist_items = [
        ...
        "[ ] Design patterns: no AI slop (check design-patterns domain)",
...
def format_markdown(design_system: dict) -> str:
    ...
    lines.append("- [ ] Design patterns: no AI slop (check design-patterns domain)")
    lines.append("- [ ] Accessibility: WCAG AA contrast, focus-visible, aria labels")
    lines.append("- [ ] Touch targets: 44x44px minimum on interactive elements")
    lines.append("- [ ] Typography: rem units, 16px minimum body, modular scale")
    lines.append("")
...
def format_master_md(design_system: dict) -> str:
    ...
    if recommended_ops:
        ...
        lines.append("## Recommended Operations")
        lines.append("")
        lines.append("Satellite skills to consider for this project:")
        lines.append("")
        for op in recommended_ops:
            op_name = op.get("name", "")
            op_desc = op.get("description", "")
            op_when = op.get("when_to_use", "")
            if op_name:
                ...
                if op_desc:
                    ...
                    lines.append("")
                if op_when:
                    lines.append(f"**When to use:** {op_when}")
                    lines.append("")
        lines.append("")
