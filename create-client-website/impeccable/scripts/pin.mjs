...
/**
 * Find harness skill directories that have impeccable available (standalone or bundled in create-client-website).
 */
function findHarnessDirs(projectRoot) {
  ...
  for (const harness of HARNESS_DIRS) {
    ...
    const bundledImpeccableSkill = join(skillsDir, 'create-client-website', 'impeccable', 'SKILL.md');
    if (existsSync(impeccableDir) || existsSync(join(skillsDir, 'i-impeccable'))) {
      dirs.push({ skillsDir, mode: 'standalone' });
    } else if (existsSync(bundledImpeccableSkill)) {
      dirs.push({ skillsDir, mode: 'bundled' });
    }
  }
  return dirs;
}
 ...
 */
function generatePinnedSkill(command, metadata, mode) {
...
This is a pinned shortcut for Impeccable \`${command}\` bundled inside **create-client-website**.

1. Read \`create-client-website/impeccable/SKILL.md\` for setup (context load, register) and shared design laws.
2. Read \`create-client-website/impeccable/reference/${command}.md\` and follow it for this invocation.

Paths are relative to the harness \`skills\` directory (sibling of this pinned skill). Pass any user arguments as the target or free-form context.
`;
  }
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with impeccable (standalone or create-client-website bundle) found.');
    return false;
  }

  let created = 0;

  for (const { skillsDir, mode } of harnessDirs) {
    const content = generatePinnedSkill(command, metadata, mode);
