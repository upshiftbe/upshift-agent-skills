...
**Bundled inside create-client-website.** `<PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>` means the directory that contains this repo’s main `SKILL.md` for create-client-website (the folder with `references/` and `assets/`, parent of `impeccable/`). Resolve it from the checkout or skills install path. Context loaders expect your shell’s **current working directory** to be the **client website project root** (where `PRODUCT.md` / `package.json` live), while the script path uses that skill root.
...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/pin.mjs <pin|unpin> <command>
```
