...
When this pack is bundled in **create-client-website**, `<PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>` is the folder that contains that skill’s root `SKILL.md` (parent of `impeccable/`). Run the loader from the **client website project root** so `PRODUCT.md` / `DESIGN.md` resolve correctly.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale earlier version.
