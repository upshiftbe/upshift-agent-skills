...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div"
```
...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/live-server.mjs stop
```
...
```bash
node <PATH_TO_CREATE_CLIENT_WEBSITE_SKILL>/impeccable/scripts/detect-csp.mjs
```
